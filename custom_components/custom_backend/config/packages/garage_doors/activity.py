...
from custom_components.custom_backend.const import CHANNEL_GARAGE_DOOR_ACTIVITY, DATA_ACTIONS, DATA_CAMERA, DATA_CHANNEL, DATA_CLOSE_GARAGE_DOOR, DATA_COLOR, DATA_FULL_NAME, DATA_GROUP, DATA_ID, DATA_IMAGE, DATA_MESSAGE, DATA_ROOM, DATA_ROOM_WITH_FREE_ACCESS, DATA_ROOM_WITH_LIMITED_ACCESS, DATA_TEXT, DATA_TITLE, DOMAIN_COVER, EVENT_TYPE_HOMEASSISTANT_START, IMAGE_GARAGE_DOOR_CLOSED_COLOR, IMAGE_GARAGE_DOOR_OPEN_COLOR, NOTIFICATION_COLOR_VIOLET, STATE_CLOSED, STATE_CLOSING, STATE_OPEN, STATE_OPENING
...
async def async_setup(hass: core.HomeAssistant, config: dict, **kwds) -> bool:
	...
	async def garage_door_state_changed(*, garage_door_slug, previous_state, new_state):
		...
		if previous_state in {STATE_CLOSED, STATE_CLOSING} and new_state in {STATE_OPEN, STATE_OPENING}:
			notify_args = {
				...
				DATA_COLOR: NOTIFICATION_COLOR_VIOLET,
	...
	async def setup_automations(event):
		for garage_door_slug in garage_doors:
			[get_garage_door_changes, subscribe_to_garage_door_changes] = window(entity_state_to_readable_store(f"{DOMAIN_COVER}.{garage_door_slug}", hass=hass), 2)
			subscribe_to_garage_door_changes(lambda *, garage_door_slug=garage_door_slug, get_garage_door_changes=get_garage_door_changes: hass.async_create_task(garage_door_state_changed(garage_door_slug=garage_door_slug, previous_state=get_garage_door_changes()[0].state, new_state=get_garage_door_changes()[1].state)))
