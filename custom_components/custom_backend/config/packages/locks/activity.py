...
from custom_components.custom_backend.const import CHANNEL_ATTACKS, CHANNEL_LOCK_ACTIVITY, CONF_ENTITY_ID, CONF_OPTION, DATA_ACTION, DATA_ACTIONS, DATA_ALARMLEVEL, DATA_ALARMTYPE, DATA_AUDIO, DATA_CAMERA, DATA_CAUSE, DATA_CHANNEL, DATA_CLOSE_GARAGE_DOOR, DATA_DATETIME, DATA_FULL_NAME, DATA_GROUP, DATA_ID, DATA_IMAGE, DATA_IN_VIEW, DATA_LOCKS, DATA_LOCK_SLOTS, DATA_MESSAGE, DATA_NICKNAME, DATA_OPEN_GARAGE_DOOR, DATA_PERSON, DATA_PHOTO, DATA_PRIORITY, DATA_PUSH, DATA_REQUIRE_DEVICE_UNLOCK, DATA_ROOM, DATA_ROOM_WITH_FREE_ACCESS, DATA_ROOM_WITH_LIMITED_ACCESS, DATA_SHORT_NAME, DATA_TEXT, DATA_TITLE, DATA_ZONE, DOMAIN_COVER, DOMAIN_CUSTOM_BACKEND, DOMAIN_INPUT_SELECT, DOMAIN_SENSOR, EVENT_TYPE_HOMEASSISTANT_START, IMAGE_ACCESS_DENIED_COLOR, IMAGE_FRAUD_COLOR, IMAGE_LOCK_UNLOCKED_COLOR, IMAGE_WRONG_PINCODE_COLOR, PRIORITY_NORMAL, PRIORITY_URGENT, PUSH_SOUND_AUTOUNLOCK_HAPTIC, SERVICE_CLOSE_COVER, SERVICE_OPEN_COVER, SERVICE_SELECT_OPTION, SHARED_REMOTE_UNLOCKING
...
async def async_setup(hass: core.HomeAssistant, config: dict, **kwds) -> bool:
	...
	async def lock_alarm_type_and_level_changed(*, lock_slug, alarm_type, alarm_level):
		...
		_LOGGER.warning(f"called lock_alarm_type_and_level_changed: {lock_slug} {alarm_type} {alarm_level}")
		...
		if alarm_type == "9":
			...
			if most_likely_unlocker is not None:
				...
				notify_args[DATA_GROUP] = f"{lock_data[DATA_FULL_NAME]} Activity"
			...
			if most_likely_unlocker:
				async def set_zone_and_room():
					zone_input_select_entity_id = f"{DOMAIN_INPUT_SELECT}.{most_likely_unlocker}_{DATA_ZONE}"
					...
					room_input_select_entity_id = f"{DOMAIN_INPUT_SELECT}.{most_likely_unlocker}_{DATA_ROOM}"
					await hass.services.async_call(DOMAIN_INPUT_SELECT, SERVICE_SELECT_OPTION, {
			...
			if alarm_level == "1":
				...
				# TODO: what is this?? Is it manual lock by pressing a button on the outside?
				...
		elif alarm_type == "22":
			if alarm_level == "1":
				notify_args = {
					...
					DATA_CHANNEL: CHANNEL_LOCK_ACTIVITY,
					DATA_GROUP: f"{lock_data[DATA_FULL_NAME]} Activity",
			...
			if alarm_level == "1":
				do_notify = True
				REMOTE_UNLOCKING = hass.data[DOMAIN_CUSTOM_BACKEND][SHARED_REMOTE_UNLOCKING]
				_LOGGER.warning(f"lock alarm changed: REMOTE_UNLOCKING: {REMOTE_UNLOCKING}")
				_LOGGER.warning(f"lock alarm changed: lock_slug: {lock_slug}")
				if lock_slug in REMOTE_UNLOCKING:
					...
					if time_since_remote_unlock < timedelta(seconds=5):
						do_notify = False
	...
	async def setup_automations(event):
		for lock_slug, lock_data in locks.items():
			...
			async_track_state_change(hass=hass, entity_ids=[f"{DOMAIN_SENSOR}.{lock_data[DATA_ALARMLEVEL]}"], action=lambda entity_id, from_state, to_state, *, lock_slug=lock_slug: hass.async_create_task(get_and_check_alarm_types_and_levels(lock_slug=lock_slug)))
